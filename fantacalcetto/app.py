...
DB_INIT_LOCK = Lock()
DATA_TABLES = ["players", "matches", "match_players", "award_types", "match_awards", "password_reset_requests"]
...
def clear_data_tables(connection, include_award_types=False):
    tables = ["match_awards", "match_players", "matches", "password_reset_requests", "players"]
...
def reset_identity_sequences(connection):
    if USE_POSTGRES:
        for table in ("players", "matches", "award_types", "match_awards", "password_reset_requests"):
            connection.execute(
                f"""
                select setval(
...
def import_payload(payload):
    ...
    for table in ("players", "matches", "award_types", "match_players", "match_awards", "password_reset_requests"):
