...
def current_player_role():
    ...
    return (player["app_role"] if player and "app_role" in player.keys() else "") or "member"
...
def reset_identity_sequences(connection):
    if USE_POSTGRES:
        for table in ("leagues", "players", "matches", "transfer_proposals", "award_types", "match_awards", "password_reset_requests", "league_events", "league_event_comments", "match_comments"):
            connection.execute(
                f"""
                select setval(
...
def import_payload(payload):
    ...
    for table in ("leagues", "players", "matches", "award_types", "match_players", "transfer_proposals", "match_awards", "password_reset_requests", "league_events", "league_event_comments", "match_comments"):
...
def log_league_event(title, body="", event_type="news", actor_player_id=None, visibility="all", league_id=None):
    try:
        target_league_id = league_id or current_league_id()
        execute(
            """
            insert into league_events (league_id, actor_player_id, event_type, title, body, visibility)
            values (?, ?, ?, ?, ?, ?)
            """,
            (target_league_id, actor_player_id, event_type, title, body, visibility),
        )
    except Exception:
...
def create_league():
    ...
    slug = "".join(char for char in slug if char.isalnum() or char == "-").strip("-") or uuid.uuid4().hex[:8]
    execute(
        """
        insert into leagues (name, slug, logo, primary_color, secondary_color)
        values (?, ?, ?, ?, ?)
...
def player_dashboard():
    player = current_player()
    league_id = player["league_id"] or current_league_id()
    match = featured_match()
    if match:
        maybe_auto_generate(match)
        match = featured_match()
